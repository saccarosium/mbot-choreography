...
void initMotors(uint8_t *l_motor_sn, uint8_t *r_motor_sn){
...
void initSensors(uint8_t *ultrasonic_sn, uint8_t *gyro_sn){
...
int getUsDistanceCm(uint8_t ultrasonic_sn){
	int val;
...
int getGyroDegrees(uint8_t gyro_sn){
    int val;
...
int main( void )
{
    ...
    // Measure data
    while(true){
        int distance = getUsDistanceCm(ultrasonic_sn);
        int degrees = getGyroDegrees(gyro_sn);
